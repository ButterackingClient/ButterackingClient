...
/*  63 */     Map<Integer, StreamState> streams = new TreeMap<Integer, StreamState>(this.streamComparator);
...
/*  72 */       StreamState state = this.activeStreams.put(Integer.valueOf(streamId), new StreamState(priority, remoteSideClosed, localSideClosed, sendWindowSize, receiveWindowSize));
/*     */       
/*  74 */       if (state == null) {
/*  75 */         if (remote) {
...
/*  85 */     StreamState state = this.activeStreams.remove(Integer.valueOf(streamId));
/*  86 */     if (state != null) {
/*  87 */       if (remote) {
...
/* 104 */     StreamState state = this.activeStreams.get(Integer.valueOf(streamId));
/* 105 */     return (state == null || state.isRemoteSideClosed());
/*     */   }
/*     */   
/*     */   void closeRemoteSide(int streamId, boolean remote) {
/* 109 */     StreamState state = this.activeStreams.get(Integer.valueOf(streamId));
/* 110 */     if (state != null) {
...
/* 119 */     StreamState state = this.activeStreams.get(Integer.valueOf(streamId));
/* 120 */     return (state == null || state.isLocalSideClosed());
/*     */   }
/*     */   
/*     */   void closeLocalSide(int streamId, boolean remote) {
/* 124 */     StreamState state = this.activeStreams.get(Integer.valueOf(streamId));
/* 125 */     if (state != null) {
...
/* 138 */     StreamState state = this.activeStreams.get(Integer.valueOf(streamId));
/* 139 */     return (state != null && state.hasReceivedReply());
/*     */   }
/*     */   
/*     */   void receivedReply(int streamId) {
/* 143 */     StreamState state = this.activeStreams.get(Integer.valueOf(streamId));
/* 144 */     if (state != null) {
...
/*     */     }
/*     */     
/* 154 */     StreamState state = this.activeStreams.get(Integer.valueOf(streamId));
...
/*     */     }
/*     */     
/* 172 */     StreamState state = this.activeStreams.get(Integer.valueOf(streamId));
/* 173 */     if (state == null) {
/* 174 */       return -1;
/*     */     }
...
/*     */     }
/*     */     
/* 187 */     StreamState state = this.activeStreams.get(Integer.valueOf(streamId));
...
/* 207 */     StreamState state = this.activeStreams.get(Integer.valueOf(streamId));
...
/* 214 */         StreamState streamState = e.getValue();
/* 215 */         if (streamState.getSendWindowSize() > 0) {
/* 216 */           PendingWrite pendingWrite = streamState.getPendingWrite();
...
/*     */     } 
/*     */     
/* 225 */     StreamState state = this.activeStreams.get(Integer.valueOf(streamId));
/* 226 */     return (state != null) ? state.getPendingWrite() : null;
/*     */   }
/*     */   
/*     */   PendingWrite removePendingWrite(int streamId) {
/* 230 */     StreamState state = this.activeStreams.get(Integer.valueOf(streamId));
...
/* 243 */     private final Queue<SpdySession.PendingWrite> pendingWriteQueue = new ConcurrentLinkedQueue<SpdySession.PendingWrite>();
/*     */ 
/*     */ 
/*     */     
...
/* 308 */       return this.pendingWriteQueue.peek();
/*     */     }
/*     */     
/*     */     SpdySession.PendingWrite removePendingWrite() {
/* 312 */       return this.pendingWriteQueue.poll();
/*     */     }
/*     */     
/*     */     void clearPendingWrites(Throwable cause) {
/*     */       while (true) {
/* 317 */         SpdySession.PendingWrite pendingWrite = this.pendingWriteQueue.poll();
